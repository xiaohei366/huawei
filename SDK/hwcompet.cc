...
double hw_compet::pid_update(pid_controller *pid, double setpoint, double measure)
{
	...
	// D
	pid->differentiator = (2.0f * pid->Kd * (measure - pid->pre_measure) + (2.0f * pid->tau - pid->T) * pid->differentiator) / (2.0f * pid->tau + pid->T);
	...
	while (fgets(line, sizeof(line), stdin))
	{
		...
		else
		{
			...
			case 4:
			{
				...
				if (workstation_id != -1)
					workstation_id -= pre_sum[robotId][workstation_id + 1];
				if (workstation_id < -1)
					workstation_id = -1;
	...
	while (fgets(line, sizeof line, stdin))
	{
		if (line[0] == 'O' && line[1] == 'K')
		{
			for (int robotId = 0; robotId < 4; robotId++)
			{
				...
				{
					if (robot_cluster[robotId].all_node_empty[i].bObstacle == true && robot_cluster[robotId].all_node_empty[i].symbol == '#' && robot_cluster[robotId].all_node_empty[i + 100].bObstacle == true && robot_cluster[robotId].all_node_empty[i + 100].symbol == '.' && robot_cluster[robotId].all_node_empty[i + 200].bObstacle == true && robot_cluster[robotId].all_node_empty[i + 200].symbol == '.' && robot_cluster[robotId].all_node_empty[i + 300].bObstacle == true && robot_cluster[robotId].all_node_empty[i + 300].symbol == '#')
					{
						// std::cerr<<"***************"<<std::endl;
					...
					if (robot_cluster[robotId].all_node_empty[i].bObstacle == true && robot_cluster[robotId].all_node_empty[i].symbol == '#' && robot_cluster[robotId].all_node_empty[i - 1].bObstacle == true && robot_cluster[robotId].all_node_empty[i - 1].symbol == '.' && robot_cluster[robotId].all_node_empty[i - 2].bObstacle == true && robot_cluster[robotId].all_node_empty[i - 2].symbol == '.' && robot_cluster[robotId].all_node_empty[i - 3].bObstacle == true && robot_cluster[robotId].all_node_empty[i - 3].symbol == '#')
					{
...
/*
			...
			for (int n = 9999; n >= 0; n--)
			{
				...
				if (robot_cluster[0].all_node_empty[n].bObstacle == true)
					...
					std::cerr << robot_cluster[0].all_node_object[n].symbol;
				cnt++;
			}
*/
			// 遍历机器人
			for (int robotId = 0; robotId < 1; robotId++)
			{
				...
				for (int wb_num = 1; wb_num < work_bench_cluster[robotId].size(); wb_num++)
				{
					...
					for (int m = 0; m < work_bench_cluster[robotId][wb_num].WorkBenchVec.size(); m++)
					{
						...
						work_bench_cluster[robotId][wb_num].WorkBenchVec[m].task_cnt = cnt;
					}
				}
			}
		...
		else
		{
			...
			for (int j = 0; j <= 99; ++j)
			{
				...
				else if (input[j] == '#')
				{
					...
					for (int m = 0; m < 4; m++)
						cost_map_for_robot[m][j][i] = INT_MAX;
				}
				...
				for (int robotId = 0; robotId < 4; robotId++)
				{
					...
					if (input[j] == '#')
					{
						...
						if (i + 1 <= 99 && j - 1 >= 0)
						{
							...
							if(robot_cluster[robotId].all_node_object[num + 100 - 1].node_cost != 2) robot_cluster[robotId].all_node_object[num + 100 - 1].node_cost = 500;
							else robot_cluster[robotId].all_node_object[num + 100 - 1].node_cost = 1000;
						}
						// right_up
						if (i + 1 <= 99 && j + 1 <= 99)
						{
					...
					else
					{
						...
						if (i > 0)
							robot_cluster[robotId].all_node_empty[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_empty[(i - 1) * 100 + (j + 0)]);
						if (i < 99)
							robot_cluster[robotId].all_node_empty[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_empty[(i + 1) * 100 + (j + 0)]);
						if (j > 0)
							robot_cluster[robotId].all_node_empty[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_empty[(i + 0) * 100 + (j - 1)]);
						if (j < 99)
							robot_cluster[robotId].all_node_empty[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_empty[(i + 0) * 100 + (j + 1)]);

						// We can also connect diagonally
						if (i > 0 && j > 0)
							robot_cluster[robotId].all_node_empty[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_empty[(i - 1) * 100 + (j - 1)]);
						if (i < 99 && j > 0)
							robot_cluster[robotId].all_node_empty[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_empty[(i + 1) * 100 + (j - 1)]);
						if (i > 0 && j < 99)
							robot_cluster[robotId].all_node_empty[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_empty[(i - 1) * 100 + (j + 1)]);
						...
						if (robot_cluster[robotId].all_node_object[num].bObstacle == true)
							continue;
						// if(robot_cluster[robotId].all_node_object[num].node_cost == 200 && input[j] != '.') robot_cluster[robotId].all_node_object[num].node_cost = 3;
						if (input[j] != '.' && input[j] != 'A'){
							robot_cluster[robotId].all_node_object[num].node_cost = 2;
						...
						if (i > 0)
							robot_cluster[robotId].all_node_object[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_object[(i - 1) * 100 + (j + 0)]);
						if (i < 99)
							robot_cluster[robotId].all_node_object[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_object[(i + 1) * 100 + (j + 0)]);
						if (j > 0)
							robot_cluster[robotId].all_node_object[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_object[(i + 0) * 100 + (j - 1)]);
						if (j < 99)
							robot_cluster[robotId].all_node_object[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_object[(i + 0) * 100 + (j + 1)]);

						// We can also connect diagonally
						if (i > 0 && j > 0)
							robot_cluster[robotId].all_node_object[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_object[(i - 1) * 100 + (j - 1)]);
						if (i < 99 && j > 0)
							robot_cluster[robotId].all_node_object[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_object[(i + 1) * 100 + (j - 1)]);
						if (i > 0 && j < 99)
							robot_cluster[robotId].all_node_object[num].vecNeighbours.push_back(&robot_cluster[robotId].all_node_object[(i - 1) * 100 + (j + 1)]);
...
int hw_compet::astar(sNode *nodeStart, sNode *nodeEnd, int robotId, int carry_empty_type)
{
	...
	if (carry_empty_type == 0)
		robot_cluster[robotId].all_node_empty = robot_cluster[robotId].fixed_all_node_empty;
	else
...
WorkBenchNodeForRobot hw_compet::GetRobotTarget(Robot &robot, int robotId)
{
	...
	for (int i = 1; i < work_bench_cluster[robotId].size(); ++i)
	{
		for (auto wb : work_bench_cluster[robotId][i].WorkBenchVec)
		{
			...
			robot.workbench_for_robot[i].push_back(WorkBenchNodeForRobot(wb.global_id, i, i, wb.x, wb.y, dis, wb.ori_material_status, wb.product_status, wb.remain_production_time, wb.is_execute_over, wb.workbench_route_workbench));
			if(wb.is_execute_over == true){
				std::cerr<<"********************"<<std::endl;
...
void hw_compet::astar_init_using(sNode *nodeStart, sNode *nodeEnd, int robotId, int global_id_end_point, int workbench_type, WorkBenchNode *workbench_route_start, WorkBenchNode *workbench_route_end)
{
	...
	reverse(startwb_to_endwb_route.begin(), startwb_to_endwb_route.end());
	...
	workbench_route_start-> execute_task_cnt++;
	if(workbench_route_start->execute_task_cnt == workbench_route_start->task_cnt) workbench_route_start->is_execute_over = true;
